...
class StopSalesByIngredientsInteractor:
    ...
    timezone: ZoneInfo
    stop_sales_cache: StopSalesCacheGateway
    ...
    async def execute(self) -> list[UnitStopSalesByIngredients]:
        ...
        service = StopSaleByIngredientService(stop_sales=stop_sales)
        stop_sales = await service.filter_non_existing_stop_sales(
            cache=self.stop_sales_cache,
        )

        service = StopSaleByIngredientService(stop_sales=stop_sales)
        return service.group_by_units_and_reasons(timezone=self.timezone)
